(function() {
  ...
  _.reject = function(collection, test) {
    ...
    var reject = []
    var resolve = _.filter(collection, test);
    for (var i = 0; i < collection.length; i++){
      if (!resolve.includes(collection[i])){
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (isSorted) {
      for (var i = 0; i < array.length; i++) {
        if (!iteratorVal.includes(iterator(array[array.lastIndexOf(array[i])])))
        uniq.push(array[array.lastIndexOf(array[i])]);
        iteratorVal.push(iterator(array[array.lastIndexOf(array[i])]))
